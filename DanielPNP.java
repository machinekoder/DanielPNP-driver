...
public class TinygDriver extends AbstractSerialPortDriver implements Runnable {
    ...
    private DriverStatus getAxisStatus(int axis) {
        ...
        DriverStatus status;
        status.ready = (bitmask & (1 << 0));
        status.zero_pos = (bitmask & (1 << 1));
        status.pos_error = (bitmask & (1 << 2));
    ...
    public synchronized void connect() throws Exception {
        ...
        for (int i = 1; i < 5 && !connected; i++) {
            try {
                String command = String.format("#%iv", i);
        ...
        failed = false;
        for (int i = 1; i < 5; i++) {
            String command = String.format("#%iZg", i);
            String response = sendCommand(command, 500);
        ...
        // Positiionsfehler zurücksetzen
        String response = sendCommand("3D");
        if (reponse != "3D") {
            throw new Exception("Positionsfehler zurücksetzen gescheitert");
        }

        DriverStatus status = getAxisStatus(3);
        if (!status.ready) {
            throw new Exception(String.format("Axis %i not ready", 3));
        }

        // Satz 11 aus EEPROM lesen
        response = sendCommand("3y11");
        if (response != "3y11") {
